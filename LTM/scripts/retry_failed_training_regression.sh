...
# Retry Failed Training Tasks - Regression Tasks
# This script retrains specific dataset-model combinations that had poor results
# Usage: ./retry_failed_training_regression.sh
...
# Failed regression tasks to retry: (dataset, model) pairs
...
LOG_FILE="$LOG_DIR/retry_failed_training_regression_${TIMESTAMP}.log"

# Redirect all output to log file AND console
exec > >(tee -a "${LOG_FILE}") 2>&1

echo "=========================================="
echo "Retry Failed Training Tasks - Regression"
echo "=========================================="
echo ""
...
echo "Retrying ${#FAILED_TASKS[@]} failed regression tasks with ${#SEEDS[@]} seeds each:"
