...
# AIDA Model Experiments - Relation Aggregation and Graph Layer Number Validation
# Usage: ./run_aida_relation_args.sh [base_encoder]
...
# Define relation aggregation methods
nHEADS=(1 2 4)
# Relation number of layers
# LAYERS=(1 2)
LAYERS=(3)

# Common arguments
COMMON_ARGS="--no_need_test --num_neighbors 256 256"
...
for dataset_config in "${ALL_DATASETS[@]}"; do
    ...
    # Loop through all relation aggregation methods
    for head in "${nHEADS[@]}"; do
        # Loop through all graph layer numbers
        for layer in "${LAYERS[@]}"; do
            echo "=========================================="
            echo "Running Experiment:"
            echo "  Database: $DB_NAME"
            echo "  Task: $TASK_NAME"
            echo "  Feature Layers: $layer"
            echo "  Feature Heads: $head"
            echo "=========================================="
            ...
            python -m aida.aida_run \
                ...
                --feat_layer_num $layer \
                --feat_nhead $head \
                $COMMON_ARGS || echo "FAILED: DB=$DB_NAME, Task=$TASK_NAME, Layers=$layer"
        done
    done
done